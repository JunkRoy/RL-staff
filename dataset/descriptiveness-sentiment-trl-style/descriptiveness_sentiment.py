...
if __name__ == "__main__":
    ...
    def process(row):
        ...
        for i in range(len(row["best"])):
            best_idx = row["best"][i]
            row["chosen"].append([{"role": "user", "content": row["prompt"][i].strip()}, {"role": "assistant", "content": row[f"sample{best_idx}"][i].strip()}])
            ...
            row["rejected"].append([{"role": "user", "content": row["prompt"][i].strip()}, {"role": "assistant", "content": row[f"sample{rejected_idx}"][i].strip()}])
        return row
